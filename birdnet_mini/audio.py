...
#def create_spectrogram(AD, sr):
    S = librosa.stft(AD)
    # zero_padding = tf.zeros([sr]) - tf.shapre(S)

    # S = tf.cast(S, tf.float32)
    # equal_lenght = tf.concat([S, zero_padding], 0)
    
    # spectrogram = tf.py_function(func=stft, inp=[equal_lenght], Tout=tf.float32)
    
    # print("spectÖ", spectrogram)
    ...
    print('Length of the spectrogram:', len(S_dB))
